...
def read_sesources(db: Session = Depends(database.get_db), current_user: schemas.User = Depends(auth.get_current_manager_or_admin)):
    return crud.get_resources(db)

@router.put('/{resource_id}', response_model= schemas.Resource)
def update_resource(resource_id: int, resource: schemas.Resource, db: Session = Depends(database.get_db), current_user: schemas.User = Depends(auth.get_current_manager_or_admin)):
...
def delete_resource(resource_id: int, db: Session = Depends(database.get_db), current_user: schemas.User = Depends(auth.get_current_manager_or_admin)):
