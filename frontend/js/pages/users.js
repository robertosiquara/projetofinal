...
function render(users) {
  list.replaceChildren(
    ...users.map((item) => {
      ...
      meta.textContent = `${item.username} • ${item.role}`;
...
list?.addEventListener("click", async (event) => {
  ...
  if (!button || !confirm("Excluir este usuário?")) return;

  try {
    await userService.remove(button.dataset.userId);
